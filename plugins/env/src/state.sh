...
env_capability_params_path() {
    printf '%s\n' "$(env_capability_dir "$1")/params.sh"
}
...
env_install_bash_env_profile_marker() {
    ...
    env_strip_marker "$file" "$begin" "$end"
    {
        printf '\n# Air env non-interactive Bash injection.\n'
        printf '%s\n' "$begin"
        printf 'export AIR_HOME="${AIR_HOME:-$HOME/.local/share/air}"\n'
        printf 'export AIR_STATE_HOME="${AIR_STATE_HOME:-$AIR_HOME/state}"\n'
        printf 'if [ -r "$AIR_STATE_HOME/plugins/env/enabled" ] && [ -r "$AIR_STATE_HOME/plugins/env/inject/bash-env.enabled" ] && [ -r "$AIR_STATE_HOME/plugins/env/runtime/bash_env.bash" ]; then\n'
        printf '    export BASH_ENV="$AIR_STATE_HOME/plugins/env/runtime/bash_env.bash"\n'
        printf '    . "$BASH_ENV"\n'
        printf 'fi\n'
        printf '%s\n' "$end"
    } >> "$file"
